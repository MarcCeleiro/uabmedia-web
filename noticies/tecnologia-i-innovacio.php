...
<html lang="ca">
	...
	<head>
		...
		<title>Notícies de Tecnologia i Innovacio - Actualitat - UABmèdia</title>
		...
		<div class="contingut">
			<div class="container">
					...
					<div class="row">
						<div class="col-12 overflow-hidden amaga-mobil">
							<div class="um-capcalera d-flex align-items-center">
								<h1 class="titol-principal titol-capcelera" style="font-size: 2.369em;line-height: 0.8!important">Notícies de Tecnologia i Innovacio</h1>
							</div>
						</div>
						<section> 
							<div class="container">
				...
				<section>
					...
					$GET_VARS_tecnologia = array(
						"go" => "news",
						"do" => "list"
					);

					$POST_VARS_tecnologia = array(
						"statusFilter" => 1,
					...
					$REQUEST_URL_tecnologia = $API_URL . "?" . http_build_query($GET_VARS) . "&" . http_build_query($GET_VARS_tecnologia);

					$ch_tecnologia = curl_init();
					curl_setopt($ch_tecnologia, CURLOPT_URL, $REQUEST_URL_tecnologia);
					curl_setopt($ch_tecnologia, CURLOPT_POST, true);
					curl_setopt($ch_tecnologia, CURLOPT_RETURNTRANSFER, true);
					curl_setopt($ch_tecnologia, CURLOPT_HEADER, false);
					curl_setopt($ch_tecnologia, CURLOPT_POSTFIELDS, $POST_VARS_tecnologia);

					$response_tecnologia = curl_exec($ch_tecnologia);
...
<br />
						...
						<div data-aos="zoom-in" class="row">
							...
							for ($i = 0; $i < count($data_tecnologia->list); $i++) {
							?>
							<div class="col-12 col-md-4">
								<div class="bloc-imatges position-relative clicable" onclick="window.location= '/noticia/<?php echo $data_tecnologia->list[$i]->id; ?>/<?php echo $data_tecnologia->list[$i]->title_url; ?>'">
									<div class="img-noticia noticia_flex" style="background-image:url(<?php echo $data_tecnologia->list[$i]->img_social; ?>)">
									</div>
								</div>
								<br />
								<h2 class="titol h6"><a href="/noticia/<?php echo $data_tecnologia->list[$i]->id; ?>/<?php echo $data_tecnologia->list[$i]->title_url; ?>"><?php echo $data_tecnologia->list[$i]->title; ?></a></h2>
								<div class="capitol d-flex align-items-center my-2">
									<span class="text-white"><?php echo substr($data_tecnologia->list[$i]->date_formatted, 0, -9); ?></span>
								</div>
								<br /><br />
							</div>
							<?php } ?>
